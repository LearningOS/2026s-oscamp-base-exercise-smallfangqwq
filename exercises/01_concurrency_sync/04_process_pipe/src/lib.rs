...
pub fn pipe_through_cat(input: &str) -> String {
    ...
    let mut stdin = cat_command.stdin.unwrap();
    stdin.write_all(input.as_bytes()).expect("Failed to write to cat stdin");
    drop(stdin); // Close the write end to signal EOF to cat
    let mut output = String::new();
    cat_command.stdout.unwrap().read_to_string(&mut output).expect("Failed to read from cat stdout");
    output
}
...
pub fn pipe_through_grep(pattern: &str, input: &str) -> String {
    ...
    let mut stdin = grep_command.stdin.unwrap();
    for line in input.lines() {
        writeln!(stdin, "{}", line).expect("Failed to write to grep stdin");
    }
    drop(stdin); // Close the write end to signal EOF to grep
    let mut output = String::new();
    grep_command.stdout.unwrap().read_to_string(&mut output).expect("Failed to read from grep stdout");
    output
}
